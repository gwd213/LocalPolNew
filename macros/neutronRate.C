...
void neutronRate(const std::string infile = "/sphenix/tg/tg01/coldqcd/ZDCLocalPol/test/store34578/store34578.root", int storenumber = 34578, int runnumber = 44701)
{
  ...
  int servival_num_north[7] = {0}; //[0] : Total Trigger , [1] : ZDC Trig , [2] : Multiplicity cut, [3] : ZDC 1st cut ,[4] : ZDC 2nd cut , [5] : ZDC 1st & 2nd cut, [6] : radius cut
  int servival_num_south[7] = {0}; //[0] : Total Trigger , [1] : ZDC Trig , [2] : Multiplicity cut, [3] : ZDC 1st cut ,[4] : ZDC 2nd cut , [5] : ZDC 1st & 2nd cut, [6] : radius cut
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    Nup[i] = new TH1D(Form("Nup_%s", beam[i].Data()), Form("Nup_%s", beam[i].Data()), nasymbins, -TMath::Pi(), TMath::Pi());
    Nup[i]->Sumw2();
    Ndown[i] = new TH1D(Form("Ndown_%s", beam[i].Data()), Form("Ndown_%s", beam[i].Data()), nasymbins, -TMath::Pi(), TMath::Pi());
    Ndown[i]->Sumw2();
    Sup[i] = new TH1D(Form("Sup_%s", beam[i].Data()), Form("Sup_%s", beam[i].Data()), nasymbins, -TMath::Pi(), TMath::Pi());
    Sup[i]->Sumw2();
    Sdown[i] = new TH1D(Form("Sdown_%s", beam[i].Data()), Form("Sdown_%s", beam[i].Data()), nasymbins, -TMath::Pi(), TMath::Pi());
    Sdown[i]->Sumw2();
  }
  //======================================================================//
  ...
  smdHits->SetBranchAddress("isTrig", isTrig);
  ...
  servival_num_north[0] = nentries;
  servival_num_south[0] = nentries;
  //======================================================================//
  ...
  for (int i = 0; i < nentries - clockOffset; i++)
  {
    ...
    if (isTrig[3] == false)
      continue;
    servival_num_north[1]++;
    servival_num_south[1]++;
    ...
    */
    ...
    else if (s_phi > TMath::Pi())
    {
      s_phi -= 2 * TMath::Pi();
    }
    if (showerCutN == 1)
    {
      servival_num_north[2]++;
      if (zdcN1_adc > ZDC1CUT)
      {
        servival_num_north[3]++;
        if (zdcN2_adc > ZDC2CUT)
        {
          servival_num_north[4]++;
          if (veto_NF < VETOCUT && veto_NB < VETOCUT)
          {
            servival_num_north[5]++;
            if (sqrt(n_x * n_x + n_y * n_y) > RMINCUT && sqrt(n_x * n_x + n_y * n_y) < RMAXCUT)
            {
              servival_num_north[6]++;
            }
          }
        }
      }
    ...
    if (showerCutS == 1)
    {
      servival_num_south[2]++;
      if (zdcS1_adc > ZDC1CUT)
      {
        servival_num_south[3]++;
        if (zdcS2_adc > ZDC2CUT)
        {
          servival_num_south[4]++;
          if (veto_SF < VETOCUT && veto_SB < VETOCUT)
          {
            servival_num_south[5]++;
            if (sqrt(s_x * s_x + s_y * s_y) > RMINCUT && sqrt(s_x * s_x + s_y * s_y) < RMAXCUT)
            {
              servival_num_south[6]++;
            }
          }
        }
      }
  ...
  TCanvas *ncall2 = new TCanvas("ncall2", "ncall2", 1200, 600);
  ...
  TCanvas *scall2 = new TCanvas("scall2", "scall2", 1200, 600);
  DrawCanvas(runnumber, sxy_hits_up[1], sxy_hits_down[1], gSqrtAsym[2], gSqrtAsym[3]);

  std::cout << servival_num_north[0] << std::endl;
  std::cout << servival_num_north[1] << std::endl;
  std::cout << servival_num_north[2] << std::endl;
  std::cout << servival_num_north[3] << std::endl;
  std::cout << servival_num_north[4] << std::endl;
  std::cout << servival_num_north[5] << std::endl;
  std::cout << servival_num_north[6] << std::endl;

  std::cout << servival_num_south[0] << std::endl;
  std::cout << servival_num_south[1] << std::endl;
  std::cout << servival_num_south[2] << std::endl;
  std::cout << servival_num_south[3] << std::endl;
  std::cout << servival_num_south[4] << std::endl;
  std::cout << servival_num_south[5] << std::endl;
  std::cout << servival_num_south[6] << std::endl;
  ...
  for (int i = 0; i < 7; ++i)
  {
    ...
    h1->SetBinContent(i + 1, (double)servival_num_south[i] / servival_num_south[0]);
  }
  c1->cd();
  c1->SetLogy();
  h1->Draw("HIST");
  ...
  c1->SaveAs("histogram_south.png");

  for (int i = 0; i < 7; ++i)
  {
    h2->GetXaxis()->SetBinLabel(i + 1, binLabels[i]);
    h2->SetBinContent(i + 1, (double)servival_num_north[i] / servival_num_north[0]);
  }
  c2->SetLogy();
  c2->cd();
  h2->Draw("HIST");
  ...
  c2->SaveAs("histogram_north.png");

  TFile *ofile = new TFile("sinasym.root", "RECREATE");
  ofile->cd();
  ncall->Write();
  scall->Write();
  ncall2->Write();
