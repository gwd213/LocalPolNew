...
int ZDCNeutronLocPol::Init(PHCompositeNode * /*topNode*/)
{
  ...
  smdHits->Branch("isTrig", isTrig, "isTrig[64]/O");
  ...
  // smdHits->Branch("smdS3_adc", &smdS3_adc, "smdS3_adc/F");
  // smdHits->Branch("smdS4_adc", &smdS4_adc, "smdS4_adc/F");
  // smdHits->Branch("smdS5_adc", &smdS5_adc, "smdS5_adc/F");
  ...
  // smdHits->Branch("smdN3_adc", &smdN3_adc, "smdN3_adc/F");
  // smdHits->Branch("smdN4_adc", &smdN4_adc, "smdN4_adc/F");
  // smdHits->Branch("smdN5_adc", &smdN5_adc, "smdN5_adc/F");
  // smdHits->Branch("smdN6_adc", &smdN6_adc, "smdN6_adc/F");
  // smdHits->Branch("smdN7_adc", &smdN7_adc, "smdN7_adc/F");

  smdHits->Branch("smdS_h_adc", smdS_h_adc, Form("smdS_h_adc[%d]/D", kNumberOfHSMD));
  smdHits->Branch("smdN_h_adc", smdN_h_adc, Form("smdN_h_adc[%d]/D", kNumberOfHSMD));
  smdHits->Branch("smdS_v_adc", smdS_v_adc, Form("smdS_v_adc[%d]/D", kNumberOfVSMD));
  smdHits->Branch("smdN_v_adc", smdN_v_adc, Form("smdN_v_adc[%d]/D", kNumberOfVSMD));

  // smdHits->Branch("smdS1_v_adc", &smdS1_v_adc, "smdS1_v_adc/F");
  // smdHits->Branch("smdS2_v_adc", &smdS2_v_adc, "smdS2_v_adc/F");
  // smdHits->Branch("smdS3_v_adc", &smdS3_v_adc, "smdS3_v_adc/F");
  // smdHits->Branch("smdS4_v_adc", &smdS4_v_adc, "smdS4_v_adc/F");
  // smdHits->Branch("smdS5_v_adc", &smdS5_v_adc, "smdS5_v_adc/F");
  // smdHits->Branch("smdS6_v_adc", &smdS6_v_adc, "smdS6_v_adc/F");
  ...
  // smdHits->Branch("smdN3_v_adc", &smdN3_v_adc, "smdN3_v_adc/F");
  // smdHits->Branch("smdN4_v_adc", &smdN4_v_adc, "smdN4_v_adc/F");
  // smdHits->Branch("smdN5_v_adc", &smdN5_v_adc, "smdN5_v_adc/F");
  // smdHits->Branch("smdN6_v_adc", &smdN6_v_adc, "smdN6_v_adc/F");
...
int ZDCNeutronLocPol::process_event(PHCompositeNode *topNode)
{
  ...
  showerCutN = 0;
  showerCutS = 0;
  std::memset(isTrig, 0, sizeof(isTrig));
  uint64_t gl1_clock = -1;
  uint64_t gl1_triggervector = -1;
  ...
  if (!zdc_cont)
  {
    std::cout << "No ZDC" << std::endl;
  }
  if (!p_gl1)
  {
    std::cout << "No GL1" << std::endl;
  }
  else
  {
    gl1_clock = p_gl1->lValue(0, "BCO");
    gl1_triggervector = p_gl1->lValue(0, "ScaledVector");

    for (int i = 0; i < 64; i++)
    {
      isTrig[i] = ((gl1_triggervector & 0x1U) == 0x1U);
      count_raw[i] = p_gl1->lValue(i, 0);
      count_live[i] = p_gl1->lValue(i, 1);
      count_scaled[i] = p_gl1->lValue(i, 2);
      gl1_triggervector = (gl1_triggervector >> 1U) & 0xffffffffU;
      if (false)
      {
        std::cout << gl1_clock << isTrig[i] << count_raw[i] << count_live[i] << count_scaled[i] << gl1_triggervector << std::endl;
      }
    }
...
*/
        ...
        if (c >= 0 && c < 16)
        {
          signalFast = (ZDCTimingMin < timingFast && timingFast < ZDCTimingMax) ? signalFast : 0;
        ...
        if (c == 16)
        {
          signalFast = (VetoTimingMin < timingFast && timingFast < VetoTimingMax) ? signalFast : 0;
          veto_NF = signalFast;
          continue;
        }
        if (c == 17)
        ...
        if (c >= 48 && c < 63)
        {
          signalFast = (NorthSMDTimingMin < timingFast) ? signalFast : 0;
        ...
        if (c >= 112 && c < 127)
        {
          signalFast = (SouthSMDTimingMin < timingFast && timingFast < SouthSMDTimingMax) ? signalFast : 0;
